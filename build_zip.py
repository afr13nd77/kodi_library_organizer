...
def build_addon_zip(cfg: dict) -> None:
    ...
    with zipfile.ZipFile(output_path, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        # Collect names already in the archive to avoid duplicates
        ...
        for filename in sorted(os.listdir(SHARED_DIR)):
            ...
            # Skip if the addon already contains this file (e.g. __init__.py)
            # Normalize separators for reliable comparison
            arcname_normalized = arcname.replace("\\", "/")
            if any(n.replace("\\", "/") == arcname_normalized for n in existing_names):
                print(f"  ~ {arcname}  (shared, skipped — already in addon)")
                continue
